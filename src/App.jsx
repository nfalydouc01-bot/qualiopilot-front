  ...
  const triggerBackendSimulation = async () => {
    ...
    try {
      ...
      if (resultatFinal.erreur) {
        ...
        const nbManquantsRestants = (resultatFinal.elements_manquants || []).length - topManquants.length;
        let commentaireCourt = resultatFinal.commentaire || "";
        if (commentaireCourt.length > 180) {
          commentaireCourt = commentaireCourt.slice(0, 180).trim() + "...";
        }

        texteAffiche += `📊 Score : ${resultatFinal.score_conformite}/100\n\n`;
        texteAffiche += `✔️ Points forts :\n${topPreuves.map(p => `  • ${p}`).join('\n')}`;
        if (nbPreuvesRestantes > 0) texteAffiche += `\n  ...et ${nbPreuvesRestantes} autre(s)`;
