  ...
  const triggerBackendSimulation = async () => {
    ...
    try {
      ...
      if (resultatFinal && resultatFinal.erreur) {
        ...
        const preuves = resultatFinal.preuves_trouvees || [];
        const manquants = resultatFinal.elements_manquants || [];
        const commentaire = resultatFinal.commentaire || "";

        texteAffiche += `📊 Score : ${resultatFinal.score_conformite}/100\n\n`;
        texteAffiche += `✔️ Points forts :\n${preuves.map(p => `  • ${p}`).join('\n')}\n\n`;
        texteAffiche += `⚠️ À améliorer :\n${manquants.map(e => `  • ${e}`).join('\n')}\n\n`;
        texteAffiche += `💬 ${commentaire}`;
